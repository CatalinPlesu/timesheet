...
<div class="dropdown dropdown-end" id="theme-picker-dropdown">
	<button
	...
	{#if isOpen}
		<ul class="dropdown-content menu bg-base-200 rounded-box z-[1] w-64 p-2 shadow-lg max-h-96 overflow-y-auto">
			...
			{#each (['light', 'cupcake', 'bumblebee', 'emerald', 'corporate', 'fantasy', 'wireframe', 'cmyk', 'autumn', 'acid', 'lemonade', 'winter', 'nord'] as const) as themeOption}
				<li>
					<button
					...
					>
						<div class="flex items-center gap-2 w-full">
							<div class="flex-1">{getThemeDisplayName(themeOption)}</div>
			...
			{#each (['dark', 'synthwave', 'retro', 'cyberpunk', 'valentine', 'halloween', 'garden', 'forest', 'aqua', 'lofi', 'pastel', 'black', 'luxury', 'dracula', 'business', 'night', 'coffee', 'dim', 'sunset'] as const) as themeOption}
				<li>
					<button
						onclick={() => selectTheme(themeOption)}
						class={currentTheme === themeOption ? 'active' : ''}
